...
class TestBulkCalculation(unittest.TestCase):
    ...
    corrected_measurements_sets_by_mass: dict[float, list[MeasurementManager]] = {}
    mass_constants = [5.5]
    density = [1.2]
    volume = [6.7]
    ...
    def _assert_measurements_equal(self, measurement, answer_measurement, params, comparison_keys=None):
        ...
        for param in params:
            for key in comparison_keys:
                data_1 = np.array(answer_measurement.measured_data[Parameter.BULKS.value][key])
                data_2 = np.array(measurement.measured_data[param.value][key])
    ...
    def test_calculate_mass_susceptibility(self):
        ...
        density = self.density[self.mass_constants.index(5.5)]
        data_calc.set_bulk_calculation_strategy('mass', mass=5.5, density=density)

        start_time = time.perf_counter()
        for measurement in self.uncorrected_measurements:
            measurement.calculate_bulk(data_calc)
        duration = time.perf_counter() - start_time
        print(f"Correction time: {duration:.8f} seconds")

        self.assertTrue(True)

    def test_calculate_volume_susceptibility(self):
        data_calc = DataCalculation()
        data_calc.set_bulk_calculation_strategy('volume', volume=6.7)
